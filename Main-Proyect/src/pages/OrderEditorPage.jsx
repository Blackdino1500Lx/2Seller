...
export default function OrderEditorPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (lastRes.data) {
        ...
        setInitialItems(lastRes.data.order_items ?? [])
        setNotas(lastRes.data.notas ?? '')
      }
  ...
  async function handleConfirm() {
    ...
    if (!online) {
      setConfirmError(
        'Sin conexión. El guardado offline se implementa en la siguiente fase.'
      )
      return
    }
    ...
    <AppShell>
      ...
      {lastOrder ? (
        <div className="px-5 py-3 bg-brand-50 border-b border-brand-100 flex items-center gap-2.5">
          ...
          <p className="text-xs text-brand-800">
            Precargado desde <b>{lastOrder.folio || lastOrder.folio_local}</b> del {fmtFecha(lastOrder.fecha)}
          </p>
        </div>
      ) : (
      ...
      {confirming && (
        <div className="fixed inset-0 z-50 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center">
          <Spinner size="lg" />
          <p className="mt-4 text-sm text-slate-600">Confirmando pedido…</p>
        </div>
      )}
